...
export default class GlobalInputMessageConnector {
  ...
  _connectWithCallback(options = {}) {
    ...
    if (options.connectSession) {
      ...
      let url =
        this.url + '/global-input/request-socket-url?apikey=' + this.apikey;
      let that = this;
      codedataUtil.basicGetURL(
        url,
  ...
  _connectToSocket(options) {
    console.log(
      'Copyright © 2017-2022 Iterative Solution Limited, visit: https://github.com/global-input/global-input-message#readme',
    );
        ...
        if (registeredMessage.result === 'ok') {
          ...
          if (options.onRegisterFailed) {
            options.onRegisterFailed(
              registeredMessage.reason
                ? registeredMessage.reason
                : 'onRegisterPermission:registration failed!',
            );
          }
        }
      });
  ...
  onRegistered(options) {
    ...
    if (options.connectSession) {
      that.socket.on(
        options.connectSession + '/inputPermissionResult',
        function (data) {
      ...
      requestInputPermissionMessage.data = JSON.stringify(
        requestInputPermissionMessage.data,
      );
      if (options.aes) {
        requestInputPermissionMessage.data = codedataUtil.encrypt(
          requestInputPermissionMessage.data,
          options.aes,
        );
      } else {
  ...
  processInputPermission(inputPermissionMessage, options) {
    if (!inputPermissionMessage.data) {
      this.sendInputPermissionDeniedMessage(
        inputPermissionMessage,
        'data is missing in the permission request',
      );
      return;
    }
    try {
      inputPermissionMessage.data = codedataUtil.decrypt(
        inputPermissionMessage.data,
        this.aes,
      );
    } catch (error) {
      this.sendInputPermissionDeniedMessage(
    ...
    try {
      ...
      this.logError(
        error + ' while parsing the json data in the permission request',
      );
      this.sendInputPermissionDeniedMessage(
        inputPermissionMessage,
        'data format error in the permission request',
      );
      return;
    }
    ...
    const deny = (
      errorMessage = ' the application has denied the request to connect',
    ) => {
      this.sendInputPermissionDeniedMessage(
        inputPermissionMessage,
        errorMessage,
  ...
  grantInputPermission(inputPermissionMessage, options) {
    if (this.grantPermissionQueue) {
      this.grantPermissionQueue = this.grantPermissionQueue.filter(
        s => s.inputPermissionMessage.client !== inputPermissionMessage.client,
      );
      this.grantPermissionQueue.push({inputPermissionMessage, options});
      this.grantPermissionQueueLastModified = new Date();
      return;
    }
    let existingSameSenders = this.connectedSenders.filter(
      s => s.client === inputPermissionMessage.client,
    );
  ...
  processGrantInputPermissionQueue() {
    ...
    if (
      currentTime.getTime() - this.grantPermissionQueueLastModified.getTime() <
      200
    ) {
      setTimeout(this.processGrantInputPermissionQueue.bind(this), 300);
    } else {
      ...
      grantPermissionQueue.forEach(queueItem => {
        this._grantInputPermission(
          queueItem.inputPermissionMessage,
          queueItem.options,
        );
      });
    }
  ...
  _grantInputPermission(inputPermissionMessage, options) {
    ...
    console.log(
      ' allow-to-connect-' + this.connectedSenders &&
        this.connectedSenders.length + ' ',
    );
    options.onSenderConnected &&
  ...
  sendInputPermissionGrantedMessage(inputPermissionMessage, options) {
    ...
    if (options.initData) {
      ...
      let inputPermissionResultInString = JSON.stringify(
        inputPermissionResult.initData,
      );
      if (this.aes) {
        inputPermissionResult.initData = codedataUtil.encrypt(
          inputPermissionResultInString,
          this.aes,
        );
      } else {
  ...
  onInputPermissionResult(inputPermissionResultMessage, options) {
    ...
    if (
      this.inputAES &&
      inputPermissionResultMessage.initData &&
      typeof inputPermissionResultMessage.initData === 'string'
    ) {
      let decryptedInitData = codedataUtil.decrypt(
        inputPermissionResultMessage.initData,
        this.inputAES,
      );
        ...
        try {
          ...
          if (this.socket) {
            let receiverDisconnected = function () {
              ...
              if (
                !this.latTimeReceiverDisconnected ||
                currentTime - this.latTimeReceiverDisconnected < 200
              ) {
            ...
            this.socket.on(
              options.connectSession + '/leave',
              receiverDisconnected,
            );
            let inputSender = this.buildInputSender(
              inputPermissionResultMessage,
              options,
            );
            this.socket.on(
              options.connectSession + '/input',
              inputSender.onInput,
            );
            let that = this;
            this.socket.on(
              options.connectSession + '/output',
  ...
  sendOutputMessage(outputMessage) {
    ...
    let encryptedMessageData = codedataUtil.encrypt(
  ...
  buildInputSender(inputPermissionMessage, options) {
    ...
    let inputSender = {
        ...
        try {
          ...
          if (inputMessage.data) {
            ...
            try {
              ...
              that.logError(
                error + 'failed to parse the decrypted input content',
              );
            }
            ...
            try {
              ...
              that.logError(
                error + 'failed to parse the decrypted initData content',
              );
            }
          } else {
      ...
      onLeave: function (data) {
        ...
        let matchedSenders = that.connectedSenders.filter(
          s => s.client === leaveMessage.client,
        );
        if (matchedSenders.length > 0) {
          ...
          if (options.onSenderDisconnected) {
            options.onSenderDisconnected(
              inputSenderToLeave,
              that.connectedSenders,
            );
          }
        }
      },
  ...
  disconnectSender(inputSender) {
    ...
    this.connectedSenders = this.connectedSenders.filter(
      s => s.client !== inputSender.client,
    );
  ...
  _onInput(inputMessage, options) {
    if (inputMessage.initData) {
      if (
        options.initData &&
        options.initData.operations &&
        options.initData.operations.onInitData
      ) {
    ...
    if (typeof inputMessage.data.index !== 'undefined') {
      if (
        inputMessage.data.index < 0 ||
        initData.form.fields.length <= inputMessage.data.index
      ) {
        console.log(' index-too-big-input-message ');
        return;
      }
      if (
        initData.form.fields[inputMessage.data.index] &&
        initData.form.fields[inputMessage.data.index].operations &&
        initData.form.fields[inputMessage.data.index].operations.onInput
      ) {
        initData.form.fields[inputMessage.data.index].operations.onInput(
          inputMessage.data.value,
        );
      } else {
        console.warn(' field-index-onInput-not-present ');
      }
    } else if (typeof inputMessage.data.fieldId !== 'undefined') {
      const matchedFields = initData.form.fields.filter(
        f => f.id === inputMessage.data.fieldId,
      );
  ...
  changeGlobalInputFieldData(globalInputdata, data) {
    ...
    if (data.fieldId) {
      ...
      data.index < globalInputdata.length
    ) {
  ...
  buildPairingData(data = {}) {
    return codedataUtil.buildPairingData(
      this.securityGroup,
      this.codeAES,
      data,
    );
