...
import { EditorWithTabMenu, IconButton, DisplayBlockText, TextInputField } from "../../../components";

import { generateRandomString } from "../../../store";


const getStateFromProps = ({ importDecryptedKey, name }) => ({ encryptionKey: importDecryptedKey ? importDecryptedKey : generateRandomString(23), name: name ? name : "", errorMessage: "" });
...
export default ({ importDecryptedKey, name, importNewKey, onBack }) => {
  ...
  const onChangeName = name => setCompData({ ...compData, name, errorMessage: "" });
  ...
  const renderRandomButton = () => {
    if (importDecryptedKey) {
      ...
      return (
        <IconButton image={menusConfig.randomGenerator.menu.image}
  ...
  return (
    <EditorWithTabMenu title={manageKeysTextConfig.generateNewKey.title}
