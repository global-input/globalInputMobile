...
const getStateFromProps = ({importDecryptedKey}) => {
        ...
        if(importDecryptedKey){
           var matchedKeyItem=appdata.findEncryptionKeyByDecryptedValue(importDecryptedKey);
           if(matchedKeyItem){
...
export default ({importDecryptedKey, menuItems,onBack})=>{
        ...
        const onDeletingEncryptionKeyItem = selectedEncryptionKeyItem => setAction({...action, selectedEncryptionKeyItem,type:ACT_TYPE.DELETING_KEY});        
        
        const onQrCodeSelected = selectedEncryptionKeyItem => setAction({...action, selectedEncryptionKeyItem,type:ACT_TYPE.PASSWORD_FOR_QR_CODE});
            
        const onClipboardCopySelected = selectedEncryptionKeyItem => setAction({...action, selectedEncryptionKeyItem,type:ACT_TYPE.PASSWORD_FOR_CLIPBOARD});
        
        const toClipboardCopyComplete = ()=> setAction({...action,type:ACT_TYPE.CLIPBOARD_COPY_COMPLETE});
   ...
   const renderListItems = () => {
    ...
    const defaultMemenus=[{
        ...
        return(
        <ViewWithTabMenu menuItems={menuItems?menuItems:defaultMemenus} selected={menusConfig.manageKeys.menu}
...
const renderGenerateNewKey = () => {
    return(<GenerateNewKey onBack={toListView}
...
const renderEncryptionKeyImported = () => {
    return(<EncryptionKeyItemImported onBack={toListView}
  ...
  const renderEncryptionKeyActivated =  () => {
    return(<EncryptionKeyItemActivated onBack={toListView}
    encryptionKeyItem={action.selectedEncryptionKeyItem}/>)
  };
  const renderViewItemDetails =()=>{
    return(<ViewEncryptionKeyDetails onBack={toListView}
    encryptionKeyItem={action.selectedEncryptionKeyItem}
    activateEncryptionKey={activateEncryptionKey}
 ...
 const renderDeletingEncryptionKey = () => {
    return(<DeletingEncryptionKey encryptionKeyItem={action.selectedEncryptionKeyItem}
      deleteEncryptionKey={deleteEncryptionKeyItem}
      onBack={toBackToItemDetails}/>);
  }

  const renderPasswordForQRCode =() => {
    return(<PasswordInputView encryptionKeyItem={action.selectedEncryptionKeyItem}
  ...
  const renderQRCode = () => {
    ...
    var menuItems=[{},{
     ...
     var help2=formDataUtil.buldTextContentResolved(action.selectedEncryptionKeyItem,manageKeysTextConfig.export.qrcode.content2);
    return(
        <QRCodeView title={manageKeysTextConfig.export.qrcode.title}
...
const renderDisplayClipboardCopy = () => {
    ...
    var menuItems=[{
         ...
         onPress:()=>{onClipboardCopy(encryptedContent);}
     }];
     console.log("----------------manage key----");
    return(
        <ClipboardCopyView title={manageKeysTextConfig.export.textContent.clipboard.title}
  ...
  const renderClipboardCopyComplete = () => {
    var menuItems=[{},{
     ...
     var content=formDataUtil.buldTextContentResolved(action.selectedEncryptionKeyItem,manageKeysTextConfig.export.textContent.complete.content)
     return(
           <ViewWithTabMenu title={manageKeysTextConfig.export.textContent.complete.title}
             content={content}
             menuItems={menuItems}/>
     )
  }
    ...
    switch(action.type){        
        ...
        case ACT_TYPE.GENERATE_NEW_KEY:       return renderGenerateNewKey();
        case ACT_TYPE.IMPORTING_KEY:          return renderImportingKey();
        case ACT_TYPE.KEY_IMPORTED:           return renderEncryptionKeyImported();
        case ACT_TYPE.KEY_ACTIVATED:          return renderEncryptionKeyActivated();
        case ACT_TYPE.VIEW_ITEM_DETAILS:      return renderViewItemDetails();    
        case ACT_TYPE.DELETING_KEY:           return renderDeletingEncryptionKey();    
        case ACT_TYPE.PASSWORD_FOR_QR_CODE:   return renderPasswordForQRCode();
        case ACT_TYPE.DISPLAY_QR_CODE:        return renderQRCode();
        case ACT_TYPE.PASSWORD_FOR_CLIPBOARD: return renderPasswordForClipboard();
        case ACT_TYPE.DISPLAY_CLIPBOARD:      return renderDisplayClipboardCopy();
        case ACT_TYPE.CLIPBOARD_COPY_COMPLETE:return renderClipboardCopyComplete();    
        default: return null;
    }
