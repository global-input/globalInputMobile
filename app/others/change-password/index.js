...
export default ({content, onBack}) => {
  ...
  useEffect(() => {}, [content]);
  const setOriginalPassword = originalPassword =>
    setCompData({...compData, originalPassword});
  ...
  const onCangePassword = () => {
    ...
    if (!originalPassword) {
      onError(
        userLoginText.errorMessages.changePassword.missing.originalPassword,
      );
      return;
    }
    ...
    if (!repeatedPassword) {
      onError(
        userLoginText.errorMessages.changePassword.missing.repeatedPassword,
      );
      return;
    }
    if (newPassword !== repeatedPassword) {
      onError(
        userLoginText.errorMessages.changePassword.repeatedPasswordNotMatch,
      );
      return;
    }
    if (appdata.changePassword(originalPassword, newPassword)) {
      ...
      onError(
        userLoginText.errorMessages.changePassword.failedToChangePassword,
      );
    }
  };
  ...
  return (
    <EditorWithTabMenu
      title={userLoginText.changePassword.title}
