...
export const DeviceInputView = ({ action, setAction, onFieldChanged,
   onDisconnect,onGlobalInputDataChanged, 
    appMenu,displayNotificationMessage,onPairingData,onSaveFormData,formDataToSave}) => {
  ...
  const buildMenuItemForDeviceInput=()=>{        
        ...
        if (action.fieldSelection) {
          ...
          if (action.initData.dataType === 'qrcode') {
            menuItems.push({ menu: menusConfig.encrypt.menu, onPress: onEncryptSecret });
            menuItems.push({ menu: menusConfig.protectedEncryptionKey.menu, onPress: onEncryptionKey });
            menuItems.push({ menu: menusConfig.serviceData.menu, onPress: onPairingData });
          }
          if (formDataToSave) {
          ...
          if (appdata.hasFormContent()) {
            menuItems.push({ menu: menusConfig.selectFromFormDataList.menu, onPress: onListAllForm });
            if (action.autofill) {
              menuItems.push({ menu: menusConfig.selectMatched.menu, onPress: listMatchedForm });
            }
          }
        }
        return menuItems;
  ...
  const buildMenuItemsForExport=()=>{
    let menuItems = [{ menu: menusConfig.disconnect.menu, onPress: onDisconnect },{
  ...
  if (action.initData.action !== 'input') {
    return (<ViewWithTabMenu menuItems={appMenu}
      title="Error">
      <View style={styles.contentCenter}>
        <DisplayBlockText title={"Error"}
  ...
  var viewIds = new Set();
  return (
    <EditorWithTabMenu title={deviceInputTextConfig.title}
      titleIcon={images.app.icons.connected}
      menuItems={menuItems} selected={menusConfig.eye.menu} notificationMessage={action.notificationMessage}>
      <View style={styles.content}>
        {renders.renderFormTitle({ initData:action.initData })}
        {renders.renderGlobalInputFields({ globalInputdata:action.globalInputdata, viewIds, onGlobalInputDataChanged, fieldSelection:action.fieldSelection, onSelectField, showHideSecret:action.showHideSecret })}
        {renders.renderViews({ initData:action.initData, globalInputdata:action.globalInputdata, viewIds, onGlobalInputDataChanged, fieldSelection:action.fieldSelection, onSelectField, showHideSecret:action.showHideSecret })}
