...
export const connect = ({
  ...
  if (codedata && codedata.url) {
    loadingMessage.push(
      deviceInputTextConfig.connector.connectMessage + ':' + codedata.url,
    );
  } else {
  ...
  const onInputPermissionResult = message => {
    ...
    if (!message.initData) {
      setErrorMessage(
        deviceInputTextConfig.initDataMissing.title,
        deviceInputTextConfig.initDataMissing.message,
      );
      return;
    }
  ...
  var options = globalInputConnector.current.buildOptionsFromInputCodedata(
    codedata,
    {
  ...
  if (client) {
    ...
    options.client = 'ginput_' + globalInputConnector.current.connector.client;
...
export const sendFieldToDevice = ({globalInputConnector, field, index}) => {
  if (globalInputConnector.current) {
    if (index === undefined || index === null || index < 0) {
      globalInputConnector.current.sendInputMessage(
        field.value,
        index,
        field.id,
      );
