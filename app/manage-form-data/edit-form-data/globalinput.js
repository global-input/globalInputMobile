...
export const startConnect = ({setData, globalInputConnector, onInput}) => {
  ...
  const onSenderDisconnected = (sender, senders) =>
    onDisconnectGlobalInput({globalInputConnector, setData});

  const onFinishReceived = () =>
    onSenderDisconnected(globalInputConnector, setData);
...
export const renderDisplayQRCode = ({data, setData}) => {
  const toEditView = () => setData(d => ({...d, actionType: ACT_TYPE.EDIT}));
