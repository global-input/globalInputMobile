...
import {CopyToClipboard,ViewWithTabMenu,DisplayBlockText} from "../../components";
...
const renderAFormFieldWithHideValue = (formField,index,label) => {
    ...
    return(
             <CopyToClipboard  key={key} style={styles.itemRow} 
 ...
 const renderAFormFieldWithShowValue = (formField,index,label) => {
    ...
    var key=getMapItemKey(formField,index,label);
    return(      
              <CopyToClipboard  key={key} contentContainerStyle={styles.showFieldContainer}
...
export default ({onDelete,onEdit, onBack,formData,displayFormDataProperties}) => {
        ...
        const renderDisplayFormData = () => {
            ...
            if(show){
                    menuItems.push({
                        ...
                        onPress:()=>{onShowChanged(false)}
                        });
            }
            else{
                          menuItems.push({
                                          menu:menusConfig.showSecret.menu,
                                          onPress:()=>{onShowChanged(true)}
                                      });
            }
            if(onDelete){
                      menuItems.push({
