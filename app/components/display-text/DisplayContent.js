...
export  default class DisplayContent extends Component {
  ...
  renderTextContent(content,textStyle, key){
        if(typeof key==='undefined'){
            ...
            return (<Text style={textStyle} key={key}>{this.renderItem(content)}</Text>);
        }
        else{
                return (<Text style={textStyle} key={key}>{content}</Text>);
         }
  }
  renderLinkTextContent(content,url,textStyle, key){
      if(!this.isValidURL(url)){
            return this.renderTextContent(content,textStyle,key);
                ...
                if(typeof content==='object'){
                  return (<Text style={textStyle}  onPress={()=>{Linking.openURL(url);}}>{this.renderItem(content)}</Text>);
                }
                else{
                      return (<Text style={textStyle}  onPress={()=>{Linking.openURL(url);}}>{content}</Text>);
                }
        }
        else if(typeof content==='object'){
            return (<Text style={textStyle} key={key}  onPress={()=>{Linking.openURL(url);}}>{this.renderItem(content)}</Text>);
        }
        else{
                return (<Text style={textStyle} key={key}  onPress={()=>{Linking.openURL(url);}}>{content}</Text>);
         }
  }
  renderViewContent(content,viewStyle, key){
        if(typeof key==='undefined'){
            ...
            return (<View style={viewStyle} key={key}>{this.renderItem(content)}</View>);
        }
        else{
                return (<Text style={viewStyle} key={key}>{content}</Text>);
         }
            ...
            if(Array.isArray(item)){
                 if(item.length>this.MAX_ARRAY_LENGTH){
                   ...
                   return(
                    ...
                    if(item.url){
                        var textStyle=formDataUtil.getStyleFromItem({item,styles,data:stylesData,name:"linkText"});

                        return this.renderLinkTextContent(item.content,item.url,textStyle,key);
                    }
                    else{
                        var textStyle=formDataUtil.getStyleFromItem({item,styles,data:stylesData,name:"contentText"});
                        return this.renderTextContent(item.content,textStyle,key);
                    }

            }
            else if(item.type==='view'){
                var viewStyle=formDataUtil.getStyleFromItem({item,styles,data:stylesData,name:"viewStyle"});
                return this.renderViewContent(item.content,viewStyle,key);
            }
            else if(item.type==='row'){
                  var viewStyle=formDataUtil.getStyleFromItem({item,styles,data:stylesData,name:"rowStyle"});
                  return this.renderViewContent(item.content,viewStyle,key);
            }
            else if(item.type==='col'){
                  var viewStyle=formDataUtil.getStyleFromItem({item,styles,data:stylesData,name:"columnStyle"});
                  return this.renderViewContent(item.content,viewStyle,key);
            }
  ...
  render(){
      if(typeof this.props.content ==='object' && Array.isArray(this.props.content)){
