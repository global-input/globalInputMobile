...
export default class FormDataUtil {
  ...
  isInitDataFormData(initData){
    return initData && initData.dataType==='form' && initData.form && initData.form.id;
  }

   fieldHasContent(formField){
      return (!formField.type || formField.type==='text') && formField.id && formField.value && formField.value.trim().length>0 ;
   }
  ...
  shouldSaveFormData(formData,appdata){
          ...
          for(var i=0;i<formData.fields.length;i++){
              ...
              if(this.fieldHasContent(formField)){              
                    var matchedFormFields=matchedFormData.fields.filter(f=>f.id===formData.fields[i].id);
  ...
  convertToStoreFormData(formData, appdata){
      ...
      for(var i=0;i<storeFormData.fields.length;i++){
          ...
          if(storeFormData.fields[i].id && storeFormData.fields[i].value && storeFormData.fields[i].value.trim().length>0){
                storeFormData.fields[i].value=appdata.encryptContent(storeFormData.fields[i].value);
          }
      }
  ...
  getStyleFromItem(request){
    try{
        ...
        if(request.item && request.item.style){
            var stdata={};            
            stdata[request.name]=Object.assign({},request.data[name],item.style);
            return StyleSheet.create(stdata)[request.name]
        }
      }
      catch(error){
        console.warn(error);
