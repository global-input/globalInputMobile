...
export const forFormData = (store, formData) => {
  const allDomainMappingRecords =
    domainFormMappings.getAllDomainMappingRecords(store);
...
export const getAutoFillData = (store, initData) => {
  ...
  let formId = formDataUtil.getFormIdFromTemplateAndFields(
    initData.form.id,
  ...
  addFormToArray(
    generalUtil.searchFormDataById(allFormsData, formId),
    matchedRecords,
  );
  ...
  addFormsToArray(
    generalUtil.findFormDataByIdSuffix(allFormsData, domain),
    matchedRecords,
  );
  addFormsToArray(
    generalUtil.searchFormDataByIdPrefix(allFormsData, formId),
    matchedRecords,
  );
  if (!matchedRecords || !matchedRecords.length) {
    if (formId && formId.length > 2 && formId.startsWith('@')) {
      addFormsToArray(
        generalUtil.searchFormDataByIdPrefix(allFormsData, formId.slice(1)),
        matchedRecords,
      );
    }
    if (!matchedRecords || !matchedRecords.length) {
...
export const searchFormData = (formDataList, filterString) => {
  ...
  const matchedRecords = [];
  addFormToArray(
    generalUtil.searchFormDataById(formDataList, search),
    matchedRecords,
  );
  addFormsToArray(
    generalUtil.searchFormDataByDomainPart(formDataList, search),
    matchedRecords,
  );
  addFormsToArray(
    generalUtil.searchFormDataByIdPrefix(formDataList, search),
    matchedRecords,
  );
