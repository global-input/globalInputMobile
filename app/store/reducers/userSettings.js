    ...
    switch (action.type) {
      ...
      case ActionNames.ADD_ENCRYPTION_KEY_ITEM:
        ...
        if (encryptionItem && encryptionItem.encryptionKey) {
          encryptionKeyList = encryptionKeyList.filter(e => e.encryptionKey !== encryptionItem.encryptionKey);
      ...
      case ActionNames.DELETE_ENCRYPTION_ITEM:
        ...
        if (encryptionItemToDelete && encryptionItemToDelete.encryptionKey && encryptionItemToDelete.encryptionKey !== activeEncryptionKey) {
          encryptionKeyList = encryptionKeyList.filter(e => e.encryptionKey !== encryptionItemToDelete.encryptionKey);
        }
        return Object.assign({}, state, { encryptionKeyList });
      case ActionNames.UPDATE_ENCRYPTION_ITEM:
        ...
        var encryptionKeyList = state.encryptionKeyList;
        if (encryptionItem && encryptionItem.encryptionKey) {
          var foundIndex = encryptionKeyList.findIndex(e => e.encryptionKey == encryptionItem.encryptionKey);
          if (foundIndex >= 0) {
      ...
      case ActionNames.SET_SHOW_WELCOME_PAGE:
        return Object.assign({}, state, { showWelcomePage: action.showWelcomePage });
        ...
        for (var i = 0; i < state.savedFormContent.length; i++) {
          if (state.savedFormContent[i].id === formData.id) {
            ...
            if (state.savedFormContent[i].fields) {
              for (var k = 0; k < state.savedFormContent[i].fields.length; k++) {
                var matchedFields = formData.fields.filter(f => f.id === state.savedFormContent[i].fields[k].id);
            ...
            if (formData.fields) {
              for (var k = 0; k < formData.fields.length; k++) {
                var matchedFields = fields.filter(f => f.id === formData.fields[k].id);
                if ((!matchedFields) || (!matchedFields.length)) {
      ...
      case ActionNames.UPDATE_FORM_DATA:
        var savedFormContent = state.savedFormContent.filter(f => f.id !== action.formId);
      ...
      case ActionNames.DELETE_FORM_DATA:
        var formData = action.formData;
        var savedFormContent = state.savedFormContent.filter(m => m.id !== formData.id);
        return Object.assign({}, state, { savedFormContent });
    }
    return state;
  },
  actions: {
    appLoginContent: function (appLoginContent, activeEncryptionKey, encryptionKeyList, savedFormContent) {
      return {
...
export const getEncryptionKeyList = store => store.getState().userSettings.encryptionKeyList;

export const getActiveEncryptionKey = store => store.getState().userSettings.activeEncryptionKey;

export const getMasterEncryptionKey = store => store.getState().userSettings.masterEncryptionKey;

export const deleteEncryptionItem = (store, encryptionItemToDelete) => store.dispatch(userSettings.actions.deleteEncryptionItem(encryptionItemToDelete));

export const updateEncryptionItem = (store, encryptionItem) => store.dispatch(userSettings.actions.updateEncryptionItem(encryptionItem));

export const addEncryptionItem = (store, encryptionItem) => store.dispatch(userSettings.actions.addEncryptionItem(encryptionItem));
...
export const getAppLoginPassword = store => store.getState().userSettings.appLoginPassword;
...
export const getAllForms = store => store.getState().userSettings.savedFormContent;

export const getShowWelcomePage = store => store.getState().userSettings.showWelcomePage;

export const setShowWelcomePage = (store, showWelcomePage) => store.dispatch(userSettings.actions.showWelcomePage(showWelcomePage));

export const updateFormData = (store, formId, formData) => store.dispatch(userSettings.actions.updateFormData(formId, formData));

export const createFormData = (store, formData) => store.dispatch(userSettings.actions.createFormData(formData));

export const mergeFormData = (store, formData) => store.dispatch(userSettings.actions.mergeFormData(formData));

export const deleteFormData = (store, formData) => store.dispatch(userSettings.actions.deleteFormData(formData));

export const clearAllForms = store => store.dispatch(userSettings.actions.clearAllForms());

export const mergeFormDataList = (store, formDataList) => store.dispatch(userSettings.actions.mergeFormDataList(formDataList));

export const getFormContentById = (store, formId) => generalUtil.findFormDataById(getAllForms(store), formId);

export const searchFormDataById = (store, formId) => generalUtil.searchFormDataById(getAllForms(store), formId);
