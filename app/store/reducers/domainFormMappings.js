...
const transformer = {
  ...
  deleteADomain: (state, domain) => {
    const domainToIds = state.domainToIds.filter(
  ...
  reducer: (state = initialState, action) => {
    switch (action.type) {
      case ActionNames.ATTACH_TO_DOMAINS:
        return transformer.attacheToDomains(
          state,
          action.formId,
          action.domains,
        );
...
export const saveDomains = ({formData, store}) => {
  ...
  store.dispatch(
    domainFormMappings.actions.attachToDomains(formData.id, domains),
  );
};
...
export const updateDomains = ({formData, formId, store}) => {
  ...
  if (formId === formData.id || !formId) {
    ...
    store.dispatch(
      domainFormMappings.actions.updateAttachedDomains(
        formId,
        formData.id,
        domains,
      ),
