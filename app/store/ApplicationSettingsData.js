...
safeDecrypt = function (content, encryptionKey) {
  try {
...
export default class ApplicationSettingsData {
  ...
  isActiveEncryptionKeyEncryptedMessage(codedata) {
    return codedata && codedata.startsWith && codedata.startsWith(this.encryptedWithEncryptionKeyCodeIdentifier);
  }
  isMasterEncryptionKeyCodedata(codedata) {
    return codedata && codedata.startsWith && codedata.startsWith(this.encryptionKeyIdentifier);
  }
  isProtectedMasterEncryptionKey(codedata) {
    return codedata && codedata.startsWith && codedata.startsWith(this.protectedKeyIdentifier);
  }
  encryptWithActiveEncryptionKey(secretMessage) {
    var prefix = generateRandomString(3);
    var suffix = generateRandomString(7);
    var activeEncryptionKey = this.getDecryptedActiveEncryptionKey();
    var encryptedMessage = encrypt(prefix + secretMessage + suffix, activeEncryptionKey + this.encryptionKeyAppendedPart);
    return this.encryptedWithEncryptionKeyCodeIdentifier + encryptedMessage;
  }
  decryptEncryptionKeyText(content) {
    var content = content.slice(this.encryptionKeyExportedAsTextIdentifier.length);
    return this.decryptExportedEncryptionKey(content, null);
  }
  decryptEncryptionKey(encryptionKeyitem) {
    const userEncryptionKey = this.buildUserEncryptionKeyFromPassword(this._getLoginUserInfo().password);
  ...
  encryptWithAnEncryptionKey(secretMessage, encryptionKey) {
    ...
    if (decryptedEncryptionKey) {
      var prefix = generateRandomString(3);
      var suffix = generateRandomString(7);
      var encryptedMessage = encrypt(prefix + secretMessage + suffix, decryptedEncryptionKey + this.encryptionKeyAppendedPart);
      return this.encryptedWithEncryptionKeyCodeIdentifier + encryptedMessage;
    }
  }
  decryptCodeDataWithAnyEncryptionKey(encryptedMessage) {

    var encrypted = encryptedMessage.substring(this.encryptedWithEncryptionKeyCodeIdentifier.length);
    var decrypted = this.decryptContentWithAnyEcryptionKey(encrypted, this.encryptionKeyAppendedPart);
    if (decrypted) {
  ...
  decryptContentWithAnyEcryptionKey(encryptedContent, encryptionKeySuffix) {
    var activeEncryptionKey = this.getDecryptedActiveEncryptionKey();
    var decryptedContent = safeDecrypt(encryptedContent, activeEncryptionKey + encryptionKeySuffix);
    if (decryptedContent) {
      return decryptedContent;
    }
    var encryptionKeyList = this.getEncryptionKeyList();
    var userEncryptionKey = this.buildUserEncryptionKeyFromPassword(this._getLoginUserInfo().password);
    for (var i = 0; i < encryptionKeyList.length; i++) {
      try {
        var decryptedEncryptionKey = safeDecrypt(encryptionKeyList[i].encryptionKey, userEncryptionKey);
        if (decryptedEncryptionKey === activeEncryptionKey) {
          continue;
        }
        if (decryptedEncryptionKey) {
          decryptedContent = safeDecrypt(encryptedContent, decryptedEncryptionKey + encryptionKeySuffix);
          if (decryptedContent) {
            return decryptedContent;
          }
        }
  ...
  setSecurityGroup(securityGroup) {
    this.store.dispatch(globalInputSettings.actions.securityGroup(securityGroup));
  }
  setCodeAES(codeAES) {
  ...
  setAppLoginTimeout(appLoginTimeout) {
    this.store.dispatch(globalInputSettings.actions.appLoginTimeout(appLoginTimeout));
  }
  setPreserveSession(preserveSession) {
    this.store.dispatch(globalInputSettings.actions.preserveSession(preserveSession));
  }
  ...
  getAppLoginTimeout() {
    var appLoginTimeout = this.store.getState().globalInputSettings.appLoginTimeout;
    if (!appLoginTimeout) {
  ...
  getAllLabels() {
    return formDataUtil.getAllLabelsFromFormDataList(userSettings.getAllForms(this.store));
  }
  hasFormContent() {
  ...
  findEncryptionKeyByDecryptedValue(decryptedKey) {
    ...
    encryptionKeyList.forEach(ekey => {
      var decryptedEncryptionKey = safeDecrypt(ekey.encryptionKey, userEncryptionKey);
  ...
  getEncryptionKeyList() {
    ...
    if (!encryptionKeyList.length) {
      var encryptionKey = this._importEncryptedNewKey("This Device", activeEncryptionKey);
  ...
  getDecryptedActiveEncryptionKey() {
    var activeEncruptionKey = this._getActiveEncryptionKeyFromLoginUserInfo(this._getLoginUserInfo());
  ...
  _importEncryptedNewKey(name, encryptedEncryptionKey) {
    var encryptionItem = { createdAt: new Date(), encryptionKey: encryptedEncryptionKey, name };
  ...
  decryptPasswordEncryptedEncryptionKeyText(content, protectionPassword) {
    var content = content.slice(this.protectEncryptionKeyExportedAsTextIdentifier.length);
  ...
  decryptFormDataText(content) {
    try {
      ...
      var activeEncryptionKey = this.getDecryptedActiveEncryptionKey();
      var globalInputData = safeDecrypt(content, activeEncryptionKey + this.backupKeySuffix);
      if (!globalInputData) {
  ...
  exportFormContentAsText() {
    ...
    if (forms && forms.length) {
      const allDomainMappingRecords = domainFormMappings.getAllDomainMappingRecords(this.store);
  ...
  isPasswordEncryptedEncryptionKeyText(content) {
    ...
    return content.startsWith && content.startsWith(this.protectEncryptionKeyExportedAsTextIdentifier);
  }
  isEncryptionKeyText(content) {
    return content.startsWith && content.startsWith(this.encryptionKeyExportedAsTextIdentifier);
  }
  ...
  exportActiveEncryptionKeyWithPassword(protectionPassword) {
    var activeEncryptionKey = this.getDecryptedActiveEncryptionKey();
    return this.exportEncryptionKeyWithPassword(activeEncryptionKey, protectionPassword);
  }
  exportEncryptionKeyItemWithPassword(encryptionKeyitem, protectionPassword) {
    ...
    return this.exportEncryptionKeyWithPassword(encryptionKeyDecrypted, protectionPassword);
  }
  exportEncryptionKeyWithPassword(decryptedEncryptionKeyValue, protectionPassword) {
    var data = {
  ...
  exportEncryptionKeyItemAsText(encryptionKeyitem, protectionPassword) {
    ...
    return this.exportEncryptionKeyValueAsText(encryptionKeyDecrypted, protectionPassword);
  }
  exportEncryptionKeyValueAsText(decryptedEncryptionKeyValue, protectionPassword) {
    var keyEncrypted = this.exportEncryptionKeyWithPassword(decryptedEncryptionKeyValue, protectionPassword);
    if (protectionPassword) {
  ...
  _appLoginContent(appLoginContent, activeEncryptionKey, encryptionKeyList, savedFormContent) {
    userSettings.appLoginContent(this.store, appLoginContent, activeEncryptionKey, encryptionKeyList, savedFormContent);
  }
  ...
  activateEncryptionKey(encryptionKeyitem) {
    ...
    this._appLoginContent(appLoginContent, encryptionKeyitem.encryptionKey, null, savedFormContent);
  }
  ...
  _isActiveEncryptionKeyEncrypted() {
    ...
    return activeEncruptionKey && activeEncruptionKey.startsWith("U2Fsd") && activeEncruptionKey.length > 25;
  }
  ...
  decryptedWithPassword(encryptedContent) {
    var userEncryptionKey = this.buildUserEncryptionKeyFromPassword(this.getLoginUserinfo().password);
  ...
  changePassword(originalPassword, newPassword) {
    ...
    if (this.getAppLoginContent()) {
      try {
        var originalUserEncryptionKey = this.buildUserEncryptionKeyFromPassword(originalPassword);
        var newUserEncryptionKey = this.buildUserEncryptionKeyFromPassword(newPassword);


        var activeEncruyptionKeyDecrypted = this.getDecryptedActiveEncryptionKey();
        var encryptedActiveEncryptionKey = encrypt(activeEncruyptionKeyDecrypted, newUserEncryptionKey);
        var encryptionKeyList = this.getEncryptionKeyList();
        if (encryptionKeyList.length) {
          encryptionKeyList.forEach(ekey => {
            var decryptedEncryptionKey = safeDecrypt(ekey.encryptionKey, originalUserEncryptionKey);
            if (decryptedEncryptionKey) {
              if (decryptedEncryptionKey === activeEncruyptionKeyDecrypted) {
                ekey.encryptionKey = encryptedActiveEncryptionKey;
              }
              else {
                ekey.encryptionKey = encrypt(decryptedEncryptionKey, newUserEncryptionKey);
              }
        ...
        this._appLoginContent(appLoginContent, encryptedActiveEncryptionKey, encryptionKeyList, null);
        return true;
  ...
  userAppLogin(password) {
    ...
    if (appLoginContent) {
      try {
        ...
        if (loginUserinfo.application !== "GlobalInput" || loginUserinfo.password !== password) {
        ...
        if (this._getActiveEncryptionKeyFromLoginUserInfo(loginUserinfo)) {
          ...
          return this._oldVersionRecoverEncryptionKeyInAppLoginContent(loginUserinfo, userEncryptionKey);
        }
  ...
  */
  ...
  _oldVersionRecoverEncryptionKeyInAppLoginContent(loginUserinfo, userEncryptionKey) {
    //old version, there is a appLoginContent that does not contains the enckey.
    var activeEncryptionKey = this._getActiveEncryptionKey();
    loginUserinfo.activeEncryptionKey = safeDecrypt(activeEncryptionKey, userEncryptionKey);
    if (loginUserinfo.activeEncryptionKey) {
      this._setLoginUserInfo(loginUserinfo);
      ...
      console.log("Failed to login because it failed to decrupt the active encryption key");
      return false;
    }
  }
  ...
  userAppLoginSetup(password) {
    ...
    var activeEncryptionKey = this._getActiveEncryptionKey();
    var loginUserinfo = this._createInitialLoginUserInfo(password, activeEncryptionKey);
    this._setLoginUserInfo(loginUserinfo);
    this._updateAppLoginContentEncrytionListAndForm(loginUserinfo);

    return true;
  }
  _updateAppLoginContentEncrytionListAndForm(loginUserinfo, encryptionKeyList, savedFormContent) {
    var userEncryptionKey = this.buildUserEncryptionKeyFromPassword(loginUserinfo.password);
    var userinfoString = JSON.stringify(loginUserinfo);
    var appLoginContent = encrypt(userinfoString, userEncryptionKey);
    var encryptedActiveEncryptionKey = encrypt(loginUserinfo.activeEncryptionKey, userEncryptionKey);
    userSettings.appLoginContent(this.store, appLoginContent, encryptedActiveEncryptionKey, encryptionKeyList, savedFormContent);
  }
